...
curl -sS -u "$USER_NAME:$USER_PASS" "$BASE_URL/api/empleados/$clave" >/dev/null
curl -sS -u "$USER_NAME:$USER_PASS" "$BASE_URL/api/empleados" >/dev/null
curl -sS -u "$USER_NAME:$USER_PASS" -X DELETE "$BASE_URL/api/empleados/$clave" >/dev/null
